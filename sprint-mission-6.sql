...
SELECT pizzas.id as pizza_id, SUM(order_details.quantity)
FROM pizzas
JOIN order_details ON order_details.pizza_id = pizzas.id
GROUP BY pizzas.id;
...
FROM pizzas
WHERE price > 20
) AS p ON order_details.pizza_id = p.id;
...
SELECT pizza_id , SUM(quantity) as quantity_sum
FROM order_details
GROUP BY pizza_id
...
SELECT pizzas.id, SUM(order_details.quantity) * pizzas.price AS total_revenue 
FROM order_details
